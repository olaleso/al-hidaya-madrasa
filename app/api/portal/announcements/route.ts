...
async function emailParents(request:Request,announcement:{id:string;title:string;message:string;audience:string}):Promise<EmailDelivery>{
 ...
 const from=runtimeEnv.EMAIL_FROM||"Al-Hidaya Madrasat <admissions@alhidayaislamiccentre.org>",portalUrl=new URL("/portal",request.url).toString(),subject=`Al-Hidaya Madrasat: ${announcement.title}`;
 let sent=0;
